...
public class OrderSubThread extends Thread {
  ...
  public void run() {
    ...
    try {
      if (0 != SimulatorService.suborderthreadcounts.get(tnum-1)) {
        logger.warning("ordersubthread"+tnum+": threadcount != 0 ... terminating!");
        SimulatorService.suborderthreadcounts.decrementAndGet(tnum-1);
        return;
      }
      SimulatorService.suborderthreadcounts.incrementAndGet(tnum-1);
      if (tnum == 1) {
      ...
      logger.fine("ordersubthread"+tnum+": started threadid=" + Thread.currentThread().getId() + " ... soft HORN");
        ...
        for (int i = 0; i < keyList.size(); i++) {
          ...
          if (entry.getOrderSize() > 0) {
            ...
            try {
              ...
              threadOrdersDone++;
              SimulatorService.outstandingOrders.put(simSequenceID,Long.valueOf(ordersnap));
              ...
              if (!interrupted && SimulatorService.outstandingOrders.containsKey(OO.getOOCorrId())) {
                  logger.severe("ordersubthread"+tnum+": Order corrId "+OO.getOOCorrId()+" timed out");
                  SimulatorService.os.addFailed();
                  SimulatorService.outstandingOrders.remove(OO.getOOCorrId());
              } else {
                int ordertime = (int) ((System.nanoTime() - ordersnap) / 1000000);
          ...
          // Terminate thread if oneshot or all orders created or less than 10ms left in day
          long timeRemaining = dayEndTime - System.currentTimeMillis();
          if (oneshot || expectedOrders <= threadOrdersDone || timeRemaining < 10) {
            if (oneshot || expectedOrders <= threadOrdersDone) {
              logger.fine(String.format("ordersubthread%d: terminating completed", tnum));
            } else {
              logger.fine(String.format("ordersubthread%d: terminating early", tnum));
            }
            SimulatorService.suborderthreadcounts.decrementAndGet(tnum-1);
            return;
          }
          // compute sleep time before next order
          else {
            long timeToSleep = 10;
            long orderTimeRemaining = -1;
            if (threadOrdersDone > 0) {
              orderTimeRemaining = (totalOrderTime / threadOrdersDone) * (expectedOrders - threadOrdersDone);
    ...
    logger.fine(String.format("ordersubthread%d: ending", tnum));
    SimulatorService.suborderthreadcounts.decrementAndGet(tnum-1);
  }
}
